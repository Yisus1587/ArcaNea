...
# Provider modules (optional). Prefer packaged adapters in `src.providers` (mainline
# backend) to avoid accidentally loading debug/legacy modules placed at project root.
# Fall back to top-level `provider_jikan.py`/`provider_tmdb.py` only if packaged
...
try:
    ...
    try:
        import provider_jikan as pj  # user-provided top-level module at repo root
    except Exception:
        pj = None
try:
    ...
    try:
        import provider_tmdb as pt  # user-provided top-level module at repo root
    except Exception:
        pt = None

"""Network helpers and provider delegation.

This module exposes helper functions that the rest of the app expects:
- `obtener_episodios`, `tmdb_get_genres`, `tmdb_search`, `tmdb_search_by_type`,
- `tmdb_get_episodes`, `buscar_anime_por_titulo`, `get_episodes_for_anime`,
- `buscar_anime_candidates`.

It will delegate to `provider_jikan`/`provider_tmdb` when available, otherwise
use local HTTP fallbacks. The import strategy above allows a user-supplied
`provider_tmdb.py`/`provider_jikan.py` at the project root to override bundled
adapters in `src.providers`.
"""
...
def buscar_anime_por_titulo(titulo, folder_path=None, media=None, provider_override=None):
    ...
    provider = (provider or "jikan").strip().lower()
    if provider not in ("jikan", "tmdb"):
        provider = "jikan"
    try:
    ...
    # 1) If provider is Jikan and a folder-level MAL id exists, prefer fetching from Jikan by id (highest priority)
